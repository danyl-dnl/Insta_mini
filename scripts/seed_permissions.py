    ...
    async with SessionLocal() as session:
        ...
        for fname in features_to_seed:
            result = await session.execute(
                select(Feature).where(Feature.feature_name == fname)
            )
        ...
        for aname in actions_to_seed:
            exist = await session.execute(
                select(Action).where(Action.action_name == aname)
            )
                ...
                if fa_obj:
                    link_result = await session.execute(
                        select(RoleFeatureAction).where(
                            ...
                            RoleFeatureAction.feature_action_id
                            == fa_obj.feature_action_id,
                        )
                    )
